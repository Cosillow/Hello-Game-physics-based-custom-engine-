...
bool CollisionManager::resolveBounds(Entity& entity) const
{
    const SDL_Rect& rect = entity.getBoundingBox();
    ...
    if (rect.x < 0 || rect.y < 0 || rect.x + rect.w > windowWidth || rect.y + rect.h > windowHeight)
    {
        ...
        // Adjust the entity's position to stay within the window bounds
